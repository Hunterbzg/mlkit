...
constructor(
    ...
    for (textBlock in text.textBlocks) { // Renders the text at the bottom of the box.
      ...
      //} else {
        for (line in textBlock.lines) {
          ...
          //val priceText = PRICE_MAPS[line.text]
          //if (priceText.isNullOrBlank()) return
          val formattedPrice = String.format("%.2f", product.precioFinal)
          drawText(
            getFormattedText(formattedPrice, line.recognizedLanguage, line.confidence),//line.text
            rect,
  ...
  companion object {
    ...
    private val PRICE_MAPS = mutableMapOf(
      "CTR0030" to "$23 450",
      "TV0658" to "$101 500",
      "HE0105" to "$23 500"
    )
